...
foreach ($parentcats as $category) {
    ...
    $node_number = printSubCategories($cat_id, $class, $html_id, $node_number);
...
function printSubCategories($category_id, $row_number, $parent_id, $node_number)
{
    ...
    echo "PRINT SUBCAT PARENT: " . $parent_id;
    echo "PRINT SUBCAT child: " . nodifyNumber($node_number);
    ...
    if (!empty($children)) {
        ...
        foreach ($children as $category) {
            ...
            $node_number = printNode($category->name, array($child_HTML_class, 'emne', 'node'), $parent_id, $node_number);

            $array_categories = pushToPlacementArray($array_categories, $parent_id, $node_number);
    ...
    $temp_array = printPosts($category_id, $child_HTML_class, $parent_id, $node_number);
...
function pushToPlacementArray($array, $parent_id, $node_number)
{
...
function addCoordinates($arrayvalue, $x, $y)
{
    ...
    return array($arrayvalue, array('x' => $x, 'y' => $y));
 ...
 *
 */
function printPosts($category_id, $parent_HTML_class, $parent_id, $node_number)
{
    ...
    if ($query->have_posts()) {
        while ($query->have_posts()) {
            ...
            <?php $node_number = printNode(get_the_title(), array('side', $parent_HTML_class), $parent_id, $node_number);

            $array_of_nodes = pushToPlacementArray($array_of_nodes, $parent_id, $node_number);
...
function printNode($titel, $array_of_classes, $parent_id, $node_number)
{
    ...
    echo "<p>PARENT: " . $parent_id . "CHILD: " . nodifyNumber($node_number);
    $classes = "";
